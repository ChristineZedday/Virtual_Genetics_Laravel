...
<div id="content">
	<div id="enr">
		<form action="{{route('registration',[$animal->id])}}" method="POST" onSubmit="return checkForm()">
			...
			<input type="text" name="nom" id="nom" value="@isset($animal->nom) {{$animal->nom}}  @endisset"/>
			</div>
			<div class='form'>
			<label for="couleur">Entrez la couleur de votre animal.</label><br/>
			<input type="text" name="couleur" id="couleur" value="@isset($animal->couleur) {{$animal->couleur}}  @endisset"/>
			</div>
