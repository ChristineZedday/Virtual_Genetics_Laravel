...
class ElevageController extends Controller
{
    ...
    public function listeAnimaux ($id, $filtre)
    {
        ...
        switch ($filtre)
        {
            ...
            case 'jeunes':
                $animaux = Animal::select(['id','nom','affixe_id', 'race_id', 'sexe', 'couleur', 'taille_cm', 'date_naissance','date_achat', 'a_vendre'])
                ->where('elevage_id', $id)->where('foal', 0)->where('age_administratif','<', 2)->get(); 
            break;

            case 'foals':
                $animaux = Animal::select(['id','nom','affixe_id', 'race_id', 'sexe', 'couleur', 'taille_cm', 'date_naissance','date_achat', 'a_vendre'])->where('elevage_id', $id)->where('foal', 1)->get(); 
            break;
    ...
    public function faireSaillir ($id)
    {
        ...
        $juments = Animal::select(['id','nom','affixe_id', 'race_id', 'sexe', 'sire_id','dam_id','couleur', 'taille_cm', 'date_naissance', 'a_vendre'])
        ->where('elevage_id', $id)->where('sexe', 'f')->whereHas('StatutFemelle', function($q) use ($date) { $q->where('vide', true)->where(function ($qu) use ($date) {$qu->where('date_saillie','<>', $date)->orWhere('date_saillie', null);});})->get();
